...
fun isCnpjValid (cnpj: String) : Boolean {
    // Cleaning cnpj from non-number characters
    val cleanCnpj = cnpj.filter{ it.isDigit() }
    val coef1 = listOf(5, 4, 3, 2, 9,
                       8, 7, 6, 5, 4,
                       3, 2, 0, 0)
    val coef2 = listOf(6, 5, 4, 3, 2,
                       9, 8, 7, 6, 5,
                       4, 3, 2, 0)
    ...
    for (idx in cleanCnpj.indices) {
        valid1 += Character.getNumericValue(cleanCnpj[idx]) * coef1[idx]
        valid2 += Character.getNumericValue(cleanCnpj[idx]) * coef2[idx]
    }
    valid1 = 11 - valid1 % 11
    ...
    return (valid1 == Character.getNumericValue(cleanCnpj[12])
            && valid2 == Character.getNumericValue(cleanCnpj[13]))
}
